...
import { AllocationPanel, AttributionPanel, BlotterPanel, BrainPanel, CouncilHeatmap, DataPanel, DriftPanel, EquityPanel, IncubationPanel, PositionsPanel, RetailSummary, RiskPanel, TracePanel } from './components'
...
export default function App(){
  ...
  return <div className="app"><nav><div className="brand"><CircleDot/><b>PARQUET</b><span>SAHJONY CAPITAL</span></div><div className="ticker-tape">{data.council.slice(0,5).map(a=><span key={a.symbol}><b>{a.symbol.replace('/USD','')}</b> {a.price<1?a.price.toFixed(4):a.price.toLocaleString()} <i className={a.direction==='long'?'up':''}>{Math.round(a.conviction*100)}%</i></span>)}</div><div className="nav-actions"><span className={connected?'connected':''}><RadioTower/> {connected?'STREAM':'SNAPSHOT'}</span><button onClick={()=>setRetail(v=>!v)} aria-pressed={retail}><SlidersHorizontal/>{retail?'PRO TERMINAL':'RETAIL SUMMARY'}</button></div></nav><div className="deskbar"><div>{desks.map(d=><button key={d} className={desk===d?'active':''} onClick={()=>setDesk(d)}>{d}</button>)}</div><p>CYCLE {data.cycle} · {data.mode} · {data.broker} · <time>{new Date(data.ts).toLocaleTimeString()}</time></p></div>{retail?<RetailSummary data={data}/>:<main className="terminal-grid"><EquityPanel data={data}/><CouncilHeatmap data={data} onSelect={setSelected}/><BrainPanel data={data}/><AllocationPanel data={data}/><RiskPanel data={data}/><DataPanel data={data}/><BlotterPanel data={data}/><AttributionPanel data={data}/><DriftPanel data={data}/><IncubationPanel data={data}/><PositionsPanel data={data}/><TracePanel selected={selected}/></main>}<footer>PARQUET v1.0 · EVERY VALUE TRACEABLE · EXECUTION AUTHORITY REMAINS IN PYTHON RISK GATES</footer></div>
}
