...
def _strings(value, limit=25):
    ...
    for item in value[:limit]:
        text = str(item)[:240]
        if (
            re.search(r"private key|api[_ -]?key|secret|bearer|token", text, re.IGNORECASE)
...
def _verified_last4(mcp):
    raw_identity = str(mcp.get("account_last4", ""))
    last4 = raw_identity[-4:]
    expected = (os.getenv("BROKER_EVIDENCE_ACCOUNT_LAST4", "") or "").strip()
    if not re.fullmatch(r"\d{4}", last4) or raw_identity != last4:
        raise ValueError("verified masked Agentic identity is required")
    if expected:
        if not re.fullmatch(r"\d{4}", expected):
            raise RuntimeError("BROKER_EVIDENCE_ACCOUNT_LAST4 must contain exactly four digits")
        if last4 != expected:
            raise ValueError("broker evidence account does not match configured identity")
    return last4
...
def sanitize(payload):
    ...
    if mcp is None:
        ...
        last4 = _verified_last4(mcp)
        if mcp.get("source_id") != "robinhood-trading-mcp" or mcp.get("identity_verified") is not True:
            raise ValueError("verified masked Agentic identity is required")
    ...
    return {
        ...
        "source_status": {
            "mcp": "VERIFIED" if clean_mcp else "UNAVAILABLE",
        ...
        "mcp": clean_mcp,
        "ui": clean_ui,
        "unexplained_value": _finite(payload.get("unexplained_value"), "unexplained_value", optional=True),
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        expected = os.getenv("BROKER_EVIDENCE_PUBLISH_TOKEN", "") or os.getenv(
            "RUNTIME_STATUS_PUBLISH_TOKEN", ""
        )
        ...
        try:
            ...
            print(f"broker-evidence POST failed: {type(exc).__name__}: {str(exc)[:160]}")
            self._send(
                400,
                {
                    "error": type(exc).__name__,
