...
import { Menu, LogOut, Settings, MessageSquare, Sun, Moon, TrendingUp, Activity, Cpu, DollarSign, Globe, Crown, ShieldCheck } from 'lucide-react'
    ...
    <div className="flex items-center gap-2">
     ...
     {user ? (
      <>
       {/* Owner badge */}
       {isOwner && (
        <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 rounded-full bg-accent/[0.08] border border-accent/20">
         <Crown className="h-3 w-3 text-accent" />
         <span className="text-[10px] text-accent font-semibold tracking-tesla uppercase">Owner</span>
         {unrestricted && <ShieldCheck className="h-3 w-3 text-accent" />}
        </div>
       )}
       <div className="relative">
        <button
        ...
        >
         <div className={cn(
          "w-8 h-8 rounded-full flex items-center justify-center font-medium text-xs border",
          isOwner
           ? "bg-accent/[0.12] text-accent border-accent/30"
           : "bg-white/[0.06] text-white border-border/50"
         )}>
        ...
        {userMenuOpen && (
         <>
          ...
          />
          <div className="absolute right-0 mt-2 w-64 py-1 bg-surface-elevated border border-border rounded-2xl shadow-2xl z-50 animate-scale-in overflow-hidden">
           <div className="px-4 py-3 border-b border-border">
            <p className="text-sm font-medium text-white truncate">{user.email}</p>
            {isOwner && (
             <div className="flex items-center gap-1.5 mt-1">
              <Crown className="h-3 w-3 text-accent" />
              <span className="text-[10px] text-accent font-semibold tracking-tesla uppercase">Platform Owner — Unrestricted</span>
             </div>
            )}
           </div>
           <div className="py-1">
     ...
     <nav className="container-custom py-2 space-y-0.5">
      ...
      {isOwner && (
       <div className="flex items-center gap-3 px-4 py-2.5">
        ...
        <span className="text-sm text-accent font-semibold">Owner — Full Access</span>
       </div>
      )}
     </nav>
    </div>
